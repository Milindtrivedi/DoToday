...
**Returning to the foreground is handled explicitly.** A backgrounded app can come back hours later, so `RecommendationsView` refreshes on `.active` — but only if what is on screen is actually older than the cache TTL, so a ten-second absence spends nothing. A screen that never loaded, or that is showing an error with its own retry button, is deliberately left alone. `CitySearchView` re-reads the saved lists on `.active`, bypassing the in-memory cache, because the store is a database that a widget, share extension or CloudKit sync could change behind us.

**Actors are reentrant, and that mattered here.** `SavedCitiesUseCase` is an `actor`, but actor isolation serialises *execution*, not state across `await`. An earlier version computed a new record set, awaited the store write, then assigned the snapshot back to its cache — reinstating a stale value over a newer one. Random concurrent testing caught it, and then caught two more layers of the same problem: store writes could *overtake* one another across suspension points, leaving storage disagreeing with memory; and `reload()` could read storage while this actor's own writes were still queued and adopt that stale snapshot. The fix is threefold — every mutation reads, computes and commits **synchronously** with no `await` in between; each commit's writes are chained behind the previous commit's so persistence order matches commit order; and `reload()` drains that chain before reading. The type's doc comment spells all of this out, because it is exactly the kind of invariant a later edit breaks silently.
...
**Current status: 190 unit tests across 25 suites, plus 21 UI tests. All passing, with zero compiler warnings.** Verified from a *fresh `git clone`* — not just an incremental build — on iOS 26.4, and the unit suite additionally on iOS 26.0. Unit tests run in ~0.35 s — no sleeps, no network, no shared state.
...
| **Property / "monkey" testing** | Thousands of *random* operation sequences against the saved-cities state machine, asserting invariants after every step: recents stay capped, neither list can contain a duplicate, `isFavourite` agrees with the list it derives from, storage never accumulates an orphaned record, and the in-memory view always matches what a reload would produce. A seeded PRNG makes any counterexample reproducible. This found two real bugs the example-based tests missed. |
| **Fuzzing the scoring pipeline** | 400 randomised forecasts (including `nil`s, ±80 °C, 500 mm rain, absurd elevations) plus explicit `infinity`/`NaN` cases, asserting every score stays inside 0…1, never becomes `NaN`, and that ranking stays sorted and deterministic. |
| **Hostile input** | Non-Latin and RTL queries, emoji, SQL/HTML injection strings, a 5,000-character query, whitespace-only input, and grapheme-cluster counting. On the payload side: duplicate dates, out-of-order dates, leap days, impossible dates, a DST transition, unknown time zones, unknown JSON keys, and arrays longer or shorter than `time`. |
| **Foreground / background** | That a short absence spends no request but a stale one refreshes; that a screen which never loaded, or is showing an error, is left alone; that a failed foreground refresh keeps content; and that returning re-reads saved cities from storage without disturbing an in-flight search. |
| **Row hit targets** | That tapping the disclosure arrow *opens* the city rather than toggling its star, on both search results and the saved lists, and that tapping the star favourites without navigating. These assert on screen-relative coordinates, because a row-relative tap passes even when the arrow is broken. |
| **UI (XCUITest)** | The primary journey end-to-end: launch → search → select → ranking → expand a day breakdown; plus visiting a city adding it to recents, opening a city *from* recents (the one action that navigates and reorders the list at the same time), and clearing recents returning to the empty prompt. Plus: backgrounding and returning at three different points in the journey, the full journey driven in **both dark and light mode** via a launch argument, and a rapid-interaction stress test that hammers tab switches, stars and navigation to check the app stays responsive. All against stubbed data. |
...
- **Manual testing found a hit-target bug that every automated test had missed.** The disclosure arrow was drawn as a bare `Image` belonging to no control, so tapping it landed in dead space and SwiftUI routed it to the nearest interactive view — the star. The arrow therefore favourited the city instead of opening it. The rows are now `NavigationLink`s, which own their hit area up to the chevron by construction, with the star as a sibling button at the leading edge. Worth noting how the regression test for it went: my first version passed against the *buggy* build, because `row.coordinate(...)` is relative to the row element's own frame, which never covered the arrow. Only a screen-relative tap reproduced it. I verified fail-before/pass-after by stashing the fix.
- **A regression run caught a noisy first launch.** SwiftData stores its database in `Application Support` but does not create that directory, so a genuinely fresh install failed, logged around a hundred lines of CoreData errors, and only then recovered. The app worked either way, which is exactly why it survived every test — nothing asserts on console output. Fixed by creating the directory up front, and verified by uninstalling, reinstalling and streaming the device log: zero CoreData errors.
